...
describe('Firestore Import', () => {
  ...
  it('should import root data', async () => {
    try {
      await firestoreImport(sampleRootData, mockFirestore, mockFirestore);
  ...
  it('should fail if trying to import malformed document to root of database', async () => {
    try {
      const results = firestoreImport(sampleRootData.__collections__, mockFirestore, mockFirestore);
  ...
  it('should fail if trying to import a set of collections into a document location', async () => {
    try {
      ...
      const results = firestoreImport(sampleRootData, documentLocation, mockFirestore);
      await Promise.all([results]);
  ...
  it('should fail if document is malformed', async () => {
    try {
      ...
      const results = firestoreImport(documentDataMissingCollections, documentLocation, mockFirestore);
      await Promise.all([results]);
      expect.fail(null, 'This should not be run.');
    } catch (err) {
      if (err instanceof AssertionError) {
  ...
  it(`should not manipulate imported data object`, async () => {
    ...
    await firestoreImport({['id1']: dump}, mockFirestore.collection('user'), mockFirestore);
